...
# woodland, stable YEAR-ROUND (~1.15 pigs/ha).
#
...
#
# MANAGED-PANNAGE scenario: this woodland is taken to be selected/managed for
# pannage (chestnut-heavy or productive oak/beech). The annual-mean gross mast
# anchor is set to a defensible 18,000 kcal/ha/day -- still firmly in managed-
# pannage territory, but no longer at the theoretical ceiling of prime chestnut
# in a heavy year. Mast is strongly biennial (heavy years alternate with near-
# failures), so the ANNUAL MEAN must sit below a good-year peak, not at it.
# Drop toward 8,000-14,000 for a generic or poorly-stocked stand, and see the
# MAST-FAILURE toggle in section 7.
Y_mast_annual   <- 18000   # was 25700: that figure was near the theoretical max
#   for prime chestnut pannage in a heavy mast year.
#   As an ANNUAL MEAN across mast/non-mast years on a
#   managed oak/beech/chestnut mix it is optimistic;
#   18000 is high-but-honest. (Dominant supply lever.)
Y_fruit_annual  <-  3000   # fallen fruit / windfalls (seasonal, mostly autumn)
Y_invert_annual <-  4000   # was 5000: trimmed to avoid DOUBLE-COUNTING. The
#   larval and earthworm biomass is already credited
#   inside Y_root ("over-wintering earthworm and
#   larval biomass"); counting the same grubs in both
#   the surface-invert and root terms inflates supply.
...
Y_mast_lean     <-  2500   # was 3000: residual cached/leftover hard mast in deep
#   winter, scaled down with the lower annual anchor.
...
#   (left unchanged: this is the load-bearing winter term)
...
W   <- 115     # was 90: mean live weight of the STANDING herd (kg). The model
#   describes a stable, self-replacing, year-round herd of 200
#   head, which must structurally contain breeding sows (Mangalitsa
#   sows run 150-200 kg), at least one boar (often 200 kg+), plus
#   growers and finishers. A herd MEAN of 90 kg would imply almost
#   no adult breeding stock, contradicting "stable year-round".
#   115 is an honest breeding-herd mean. Raise toward 130-140 for a
#   herd weighted to near-finish finishers. (Dominant demand lever.)
ADG <- 300     # average daily liveweight GAIN (g/day) DURING THE GROWING
#   SEASON. Forage-paced; Mangalitsas are very slow-growing.
G   <- ADG / 1000

a <- 118       # was 125: maintenance coeff (kcal/kg^0.75/day). NRC thermoneutral
#   maintenance is ~100-106. A genuine outdoor premium is real, but
#   the Mangalitsa is the most cold-buffered pig there is (thick
#   lard + dense winter coat), so the uplift over NRC should be
#   modest. 118 keeps a real outdoor premium without over-charging.
b <- 9000      # was 8000: energy cost per kg of gain (kcal/kg). Mangalitsa gain
#   is lipid-heavy; lipid deposition costs ~12.5 kcal/g of tissue
#   laid down including conversion inefficiency, so mixed fatty gain
#   sits in the 9000-11000 range. Raised to 9000 as a conservative
#   correction consistent with "gain is mostly fat".
...
cat(" MANAGED PANNAGE, stable year-round herd\n")
cat(" (2025 stress-tested parameters)\n")
cat("=====================================================\n")
...
## ===========================================================================
# IMPORTANT REFRAMING (2025 revision):
# The previous draft was tuned to hit obs_rate exactly (model/observed = 1.00x).
# With defensible parameters the model now sits BELOW the observed stocking rate,
# and that is most likely CORRECT behaviour rather than a residual error. Almost
# every working outdoor pig system supplements feed, especially through winter
# (whey, spent grain, cull veg, bagged ration). If this farm supplements at all,
# then its observed stocking rate is partly fed off-system, and a PURE-FORAGE
# carrying-capacity model SHOULD undershoot it. 
obs_pigs <- 200
obs_ha   <- 174
...
cat(sprintf(" Implied supplementation   : %5.1f%% of annual energy\n",
            100 * (1 - N_sustainable / obs_rate)))
cat("   (only if farm is NOT pure-forage; else this is model shortfall)\n")
cat("-----------------------------------------------------\n")
