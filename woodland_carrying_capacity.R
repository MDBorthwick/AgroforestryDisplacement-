...
#                            ( a * W^0.75 ) + ( b * G )
#
# Estimates the number of forage-fed Red Ranger broilers ONE HECTARE of
...
# results.
#
# This version works on a PER-HECTARE basis throughout: every figure reported
# is birds per hectare. The four-cycle throughput block at the end models a
# full year of production on one hectare across four grow-outs.
#
...
# This model is fixed to a single hectare. All outputs are per hectare.

# ----- Forage energy supply (kcal per hectare per day) ----------------------
# GROSS edible energy birds could in principle obtain, by source, BEFORE the
# accessibility and digestibility multipliers below are applied.
# Give two scenarios: annual mean and the worst (lean) season.
#
# ANCHOR FOR MAST: a well-stocked oak stand drops on the order of 200-300 kg
# acorns/ha/yr in a reasonable (non-failure) year. At ~4 kcal/g this is only
# ~3,000 kcal/ha/day averaged over the year from acorns alone. Adding beech
# mast, hazel and sweet chestnut in a mixed canopy, and counting what is
# reachable rather than only the literal ground-drop, supports an annual-mean
# GROSS mast figure of roughly 8,000 kcal/ha/day in a decent nut year. The old
# value of 45,000 implied ~4,100 kg acorns/ha/yr, which is physically
# implausible for any real stand, so it has been cut to 8,000.

# Annual-average daily yields:
Y_mast_annual   <- 8000    # nuts / acorns / chestnut / beech (energy backbone)
Y_fruit_annual  <- 3000    # fallen fruit / windfalls (seasonal, mostly autumn)
Y_invert_annual <- 4500    # insects, larvae, worms (key protein source)
Y_herb_annual   <- 4000    # herbaceous ground forage (low-energy filler)

# Worst-season (lean) daily yields  -- mid-winter, temperate.
# KEY CORRECTION: by deep winter the autumn acorn drop has largely been eaten,
# rotted or cached by other wildlife, and NO new mast falls. Standing mast in
# winter is therefore residual carry-over only, not zero but small. Fruit is
# genuinely zero (all windfalls gone). Invertebrate activity collapses in cold
# soil. Herbaceous forage persists at a low evergreen/rosette level.
Y_mast_lean     <- 2000    # residual cached/leftover hard mast only
Y_fruit_lean    <-    0    # no windfalls remain in deep winter
Y_invert_lean   <-  800    # soil fauna largely dormant in the cold
Y_herb_lean     <- 1500    # evergreen ground forage persists at low level

# ----- Diversity multipliers (each 0 to 1) ----------------------------------
# Lowered slightly: most real woodlands are not optimally structured for a
# long, even forage season, and invertebrate support is rarely maximal.
D_t  <- 0.55   # temporal-evenness: higher when species mature across the season
D_p  <- 0.75   # pollinator/invertebrate support: higher with early+late flowering

# ----- Accessibility and utilisation (each 0 to 1) --------------------------
# A_hab lowered: in closed woodland a large share of forage falls into dense
# understorey, brash, leaf litter or steep/wet ground birds cannot work. Around
# half reachable is generous rather than conservative.
# eps lowered: forage is fibrous and variable, so metabolizable-energy capture
# is well below the ~0.75-0.80 typical of a formulated concentrate diet.
A_hab   <- 0.45   # fraction of forage physically reachable by birds
A_breed <- 0.75   # fraction of reachable forage a breed actually captures
                  #   Red Rangers forage actively, so this stays high
eps     <- 0.65   # digestibility / metabolizable-energy utilisation fraction
...
ADG <- 25      # average daily liveweight GAIN (grams/bird/day).
               #   LOWERED from 35. A forage-only diet cannot sustain
               #   concentrate-fed growth rates; ~25 g/day (~2.4 kg over ~95
               #   days) is a more honest figure for slow-growing birds on
               #   woodland forage. Raise it only if you supplement feed.
G   <- ADG / 1000   # daily gain in kg/bird/day (used by the model)

# MAINTENANCE coefficient. Published broiler/breeder maintenance sits around
# 98-104 kcal/kg^0.75/day in a thermoneutral cage. Free-ranging woodland birds
# expend substantially more on locomotion, foraging and thermoregulation in an
# uncontrolled outdoor environment, so this is lifted from 100 to 130.
a <- 130       # allometric maintenance coefficient (kcal per kg^0.75 per day)

# COST OF GAIN. Energy retained per kg of liveweight gain divided by the
# efficiency of ME use for gain (k_g ~ 0.6). Published broiler MEg values are
# ~5,600-5,800 kcal per kg of gain. RAISED from 3,500, which understated the
# true energetic cost of putting on liveweight and so inflated bird numbers.
b <- 5600      # energy cost per kg of liveweight gain (kcal/kg gain).
               #   Re-derive if you change strain or diet density.
...
## ===========================================================================
## 6. FOUR-CYCLE ANNUAL THROUGHPUT  (one hectare)
## ===========================================================================
# Models a full year on one hectare across four grow-outs:
#   1 cycle at winter (lean) capacity
#   1 cycle at summer (peak) capacity   -- annual-average yields as stand-in
#   2 cycles at middling capacity       -- midpoint of lean and annual
#
# REALITY CHECK: with forage only, the winter cycle carries close to one bird
# per hectare. In practice you would not run a true winter grow-out on forage
# alone; you would either skip it, supplement with feed, or align grow-outs to
# the mast and invertebrate flush. The winter figure is kept here to show the
# binding constraint, not as an endorsement of winter forage-only finishing.

# Middling yields: midpoint of lean (winter) and annual-average (summer peak)
Y_mast_mid   <- (Y_mast_annual   + Y_mast_lean)   / 2
Y_fruit_mid  <- (Y_fruit_annual  + Y_fruit_lean)  / 2
Y_invert_mid <- (Y_invert_annual + Y_invert_lean) / 2
Y_herb_mid   <- (Y_herb_annual   + Y_herb_lean)   / 2

N_winter <- N_lean    # winter cycle = lean capacity (already computed)
N_summer <- N_annual  # summer cycle = annual-average capacity (peak stand-in)
N_mid    <- carrying_capacity(Y_mast_mid, Y_fruit_mid, Y_invert_mid, Y_herb_mid,
                              D_t, D_p, A_hab, A_breed, eps, a, W, b, G)$N

# Per hectare, birds per cycle equal the per-ha density for that cycle.
cycles_N <- c(Winter = N_winter, Summer = N_summer,
              Spring = N_mid,    Autumn = N_mid)

cat("\n=====================================================\n")
cat(" FOUR-CYCLE ANNUAL THROUGHPUT  (per hectare)\n")
cat("=====================================================\n")
cat(" 1x winter (lean) + 1x summer (peak) + 2x middling\n")
cat("-----------------------------------------------------\n")
for (nm in names(cycles_N)) {
  cat(sprintf(" %-7s : %6.1f birds\n", nm, cycles_N[nm]))
}
cat("-----------------------------------------------------\n")
cat(sprintf(" TOTAL ANNUAL CHICKEN OUTPUT : %.1f birds/ha\n", sum(cycles_N)))
cat("=====================================================\n")
#Find me at https://github.com/MDBorthwick/AgroforestryDisplacement-.git
